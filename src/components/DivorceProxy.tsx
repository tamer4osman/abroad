// src/components/DivorceProxy.tsx
...
interface FormData {
  ...
  wifeName: string; // Specific to Divorce Proxy
}
...
// --- Form State Hook ---
const useDivorceProxyFormState = () => {
...
const DivorceProxy: React.FC = () => {
  ...
  const handleSubmit = useCallback(
    async (e: React.FormEvent) => {
      ...
      console.log("Divorce Proxy Data:", formData);
  ...
  const purposeSection = useMemo(
    () => (
      <Section title="موضوع التوكيل والغرض منه">
        ...
        <p className="text-md text-gray-700 dark:text-gray-300 leading-relaxed">
          المثول أمام المحاكم والجهات الخاصة والعامة، بخصوص رفع دعوى طلاق أو
          اثبات طلاق أو التلفظ به للزوجة:
        </p>
        <InputField
          label="اسم الزوجة المعنية بالطلاق"
          id="wifeName"
          name="wifeName"
    ...
    [formData.wifeName, handleChange]
  );
  ...
  return (
    ...
    >
      ...
      <div className="hidden print:block mb-4 text-center">
        <h1 className="text-xl font-bold">توكيل طلاق</h1>
      </div>
